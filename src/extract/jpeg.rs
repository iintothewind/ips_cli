...
const SOS_TYPE: u8 = 0xDA; // Start of Scan — image data begins after this
const COM_TYPE: u8 = 0xFE; // Comment segment
const APP1_TYPE: u8 = 0xE1; // APP1 (XMP or EXIF)
...
pub fn extract(path: &Path, verbose: bool) -> Vec<PromptRecord> {
    ...
    loop {
        // Every segment starts with 0xFF
        ...
        // SOS = image data starts, no more metadata segments
...
/// Extract dc:description content from XMP XML bytes using string search.
///
/// Handles the typical AI generator XMP structure:
/// `<dc:description><rdf:Alt><rdf:li xml:lang="x-default">TEXT</rdf:li>`
